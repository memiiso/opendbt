...
class OpenDbtTableRef:
    ...
    def table_fqn(self) -> str:
        ...
        return ".".join([*db_structure.values(), self.table])

    def _generate_db_structure(self):
        
        db_structure = {}
        if self.dialect != Dialect.STARROCKS.value:
            db_structure["database"] = self.database
            db_structure["schema"] = self.schema
            return db_structure
...
class OpenDbtNode(OpenDbtLogger):
    ...
    def db_schema_dict(self, include_parents=True) -> dict:
        db_structure = {}
        schema, table = self.table_ref.schema, self.table_ref.table
        db_structure[schema] = {}
        db_structure[schema][table] = {}
        for col_name, col in self.columns.items():
            db_structure[schema][table][col_name] = col.type
